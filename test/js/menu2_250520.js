$(document).ready(function(){
    ...
    $(window).scroll(function(){ //스크롤 할대마다 1번씩 실행
        scroll_chk() //함수의 실행
    })//$(window).scroll
    ...
    */
    ...
    $('header .gnb .gnb_wrap ul.depth1 > li:last-child > ul.depth2 > li:last-child  > a').on('focuseout', function(){
        if(device_status == 'pc'){
            $('header').removeClass('menu_over')
            $('header .gnb .gnb_wrap ul.depth1 > li').removeClass('over')
        }
    ...
    /*
       모바일에서만 
       메뉴 열기 버튼을 클릭하면 header에 menu_open클래스 추가
        >>header .gnb .gnb_open
       메뉴 닫기 버튼을 클릭하면 header에 menu_open클래스 삭제
