$(document).ready(function(){
    ...
    /*
        ...
        3. 다른 메뉴에 마우스를 오버하면 이전에 오버했던 메뉴는 아웃됨( over클래스삭제)
           removeClass
    ...
    */
    ...
    $('header .gnb .gnb_wrap ul.depth1 > li').on('mouseenter', function(){
        if(device_status == 'pc'){ //pc일때만 실행
            ...
            일단 모든 li에 있는 over클래스를 모두지우고
            마우스를 오버한 li에만 over을 추가함
            이럼 이전에 오버했던 li를 정확하게 몰라도됨*/
            $('header .gnb .gnb_wrap ul.depth1 > li').removeClass('over')
            $(this).addClass('over')
        }
    ...
    $('heder .tnb .search').on('focusin', function(){
        $('header').removeClass('menu_over')
        $('header .gnb .gnb_wrap ul.depth1 > li').removeClass('over')
    })
    ...
    /*
        ...
        메뉴가 닫혀있는 상태라면 다른메뉴를 닫고 나만 열음
        ::메뉴를 클릭했을때 현재 메뉴가 열려있는지 닫혀있는지 알아야함
        1차 메뉴 li에 open클래스가 있으면 열려있는것
        hasClass
    ...
    */
    ...
    /*
        ...
        메뉴닫기 버튼을 클릭하면 메뉴닫힘
        --header .gnb .gnb_close를 클릭하면 header에 menu_open클래스 삭제
    */
